...
async def create_customer(customer_data: CustomerCreate, session: SessionDep):
    customer = Customer.model_validate(customer_data.model_dump()) # model_dump() is used to convert the Pydantic model to a dictionary
...
async def update_customer(customer_id: int, customer_data:CustomerUpdate, session: SessionDep):
    ...
    if not customer_db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Customer does not exist")
    # This function retrieves a customer from the database using the provided customer_id.
    customer_data_dict = customer_data.model_dump(exclude_unset=True) # model_dump() is used to convert the Pydantic model to a dictionary
